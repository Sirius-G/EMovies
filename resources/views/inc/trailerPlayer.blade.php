`<div class="video_section">
    ...
    <div class="margin-top: 0px;" id="video_wrapper">
        ...
        <div class="video_title" style="position: absolute; top: 80px; left: 50px; color: #fff; font-size: 25px; font-famly: tahoma, sans-serif;">{{$v->title}} Trailer</div>
        @if(strpos($v->trailer, "https://") !==false)
            @php $video_source = $v->trailer; $poster = '/assets/posters/'.$v->poster; @endphp
        ...
        @else 
            @php $video_source = '/assets/videos/'.$v->trailer; $poster = '/assets/posters/'.$v->poster; @endphp
            <video id="elevationmovies" style="margin: 2.5% 0px 0px 10%; width: 80%; height: calc(80vw*0.5625)px;" playsinline poster="{{asset($poster)}}">
              <source id="source_id" src="{{asset($video_source)}}" type="video/mp4">
                Your browser does not support HTML5 Video.
            </video>
